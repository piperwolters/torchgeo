...
class UNetDecoder(nn.Module):
    ...
    This mirrors ``rslearn.models.unet.UNetDecoder`` for the single-scale case.
    The OlmoEarth ViT backbone returns a *single* feature map (at 1/patch_size
    resolution), so -- exactly as in rslearn when its ``in_channels`` has one
    entry -- there are no multi-resolution skip connections; this is a plain
    progressive upsampler, not a true U-Net. To match rslearn's behavior (and
    its results) the same choices are kept here:

    * full-width (``in_dim``) channels through every stage -- no tapering,
    * one 3x3 conv + ReLU per upsample stage (``conv_layers_per_resolution``),
      with one leading conv at the lowest resolution and a 3x3 output conv,
    ...
    * **no normalization** (no BatchNorm).
    ...
    def __init__(
        ...
        Args:
            in_dim: Number of input feature channels (ViT embedding size).
            num_classes: Number of output channels (classes or regression targets).
            patch_size: Downsampling factor of the input feature map; must be a
                power of two.
            conv_layers_per_resolution: Number of 3x3 conv + ReLU blocks applied
